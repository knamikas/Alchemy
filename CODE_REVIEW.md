...
src/driver/          ← pool.py, resume.py, writers.py, progress.py, runlog.py, resources.py
...
**Every phase is complete** — the production-readiness gate, the coverage work the restructuring depends on, the mechanical cleanups, all six `main.py` extractions, all four of the `bond_analysis.py` splits, and all three typed contracts. **All 26 commits are done** — twenty-five landed, the last in the working tree.
...
| 13 · `Extract the worker entry point` | `7f9a86b` | 13 functions, 7 constants and the two per-worker globals out of `main.py`, verified AST-identical to the originals except one token: `_initial_result` now stamps `_version.__version__` directly rather than reaching for the driver's `ALCHEMY_VERSION`. `worker.py` holds the whole per-entry pipeline plus the worker half of the liveness protocol; the driver keeps its half (`_drain_inflight`, `_dead_worker_pids`, `_shutdown_pool`) and the run-provenance stampers, which run once before the pool exists. `main.py` 1,193 → 991 statements. Four mutations tried, three confirmed caught; the fourth is noted below. |
| 14 · `Extract the driver's output machinery` | `002c3c9` | `driver/` package: `writers.py` (both CSV schemas, `_manifest_row`, `_OutputWriters`), `progress.py`, `runlog.py`, and a fourth module the plan did not name — `resources.py`, for `available_cpu_count`, `available_memory_bytes` and `automatic_worker_limits`. The split forces it: `_RunLog` stamps two of those probes into every log, and leaving them in `main.py` would have made `driver.runlog` import the module that imports it. All seven definitions AST-identical to the originals. `main.py` 991 → 698 statements. Five mutations tried; the two that survived — the progress throttle and the slowest-entries ordering — now have tests, so all five are caught. |
| 15 · `Extract resume handling and decompose the driver` | `d29cf76` | `driver/resume.py` takes the eight functions and one class that decide what a resumed run may reuse, all AST-identical. Then `_run` — 597 lines, not the ~250 the plan predicted — becomes a 6-line handler over `_execute`, a 29-line orchestrator naming thirteen phases. Three small carriers replace the ~25 locals that were threaded through it: `_OutputLayout`, `_ConfidencePlan`, `_BatchTally`. The per-phase `run_log.driver_error` / `logger.error` / `return 1` triple collapses onto the existing `_DriverError` and one handler, which partly anticipates 3.10; `SystemExit` and the exit-code policy are left for commit 26. Verified by comparing every string literal in the old `_run` against the new phases: no message changed. `main.py` 698 → 610 statements. Seven mutations tried, six caught; the survivor only leaks a staging directory the next run's sweep removes. The new carriers and `_OutputWriters`' two confidence writers carry explicit `Optional` annotations: each starts at `None` for the disabled case, which a type checker otherwise infers as the attribute's whole type and then rejects every later assignment. |
| 16 · `Extract the command-line interface` | `5dc8f52` | `cli.py` takes `parse_args`, its two argparse type callables, `main` and the SIGTERM handler; `driver/pool.py` takes everything else — `_run`, `_execute`, the phases, the CCP4 boundary, run provenance and pool supervision. `src/main.py` is 49 lines: the module docstring the README is checked against, and `from cli import main`. Every definition AST-identical. **Larger than the plan's 160 lines, because the plan's own two halves could not both hold**: if `main` moves to `cli` then `cli` imports the driver, so `main.py` cannot also delegate to `cli` without a cycle — the driver had to move too, which the 2.1 target layout already anticipated by naming `driver/pool.py`. `REPO_DIR` is now imported from `ccp4_setup` rather than recomputed: the two-`dirname` walk is relative to the file running it, so repeating it one directory deeper in `driver/` silently named `src/` as the checkout. Six mutations tried, five caught; one test added for the `REPO_DIR` depth trap and one asserting `main.main is cli.main`. ~150 `main.X` references across seven test modules repointed at their owners. |
| 17 · `Extract the DPI calculation and EDSTATS sigma helpers` | `9119c52` | `dpi.py` takes the four functions behind the Blow precision index; `_sigma_index`, `ZD_COLUMNS`, `_zd_indices` and `_sigma_for` join `metal_identification`, so the module that defines `EDSTATS_COLUMNS` is also the one that reads them back. All seven AST-identical. `bond_analysis`' second `NAN = float("nan")` is deleted rather than copied into `dpi.py`: `structure_analysis` already defined one, and both modules already import from it. `bond_analysis.py` 2,204 → 2,035 lines, 731 → 640 statements. Four mutations tried, all four caught. |
| 18 · `Load reference data on demand instead of at import` | `c311765` | `reference_data.py` owns `DATA_DIR`, both bundled files, and `lru_cache`d accessors returning `frozenset`/`MappingProxyType`. **The only behavioural commit in Phase F.** Importing `bond_analysis` no longer reads anything, so `--help`, test collection and every spawned worker stop paying for it, and a malformed catalog now raises from the call that wanted it rather than out of an import statement. The two disagreeing catalog parsers become one pass with one set of rules — a short legacy row is now a known component and never a classification, where before it was valid to one reader and fatal to the other. `first_sphere_targets` is built inside a function, which removes the four loop variables the module-scope loop left behind; `AA` is frozen like its neighbours. All four datasets verified byte-identical to the old parsers on the bundled files. `bond_analysis.py` 2,035 → 1,974 lines, 640 → 604 statements. New `tests/test_reference_data.py`, 12 tests; seven mutations tried, all seven caught. |
| 19 · `Extract declared-connection resolution` | `03e3801` | `declared_connections.py` takes the eight functions that bind a source declaration to the analysis model. Seven are AST-identical; the eighth is the review's worst function — `_collect_declared_candidates`, 159 lines at nesting depth 6 — and it is decomposed as 2.3 asked, into `_resolve_declared_partners` (the two-phase metal test and the conformer re-pointing) and `_declared_candidate_for_connection` (the six guards, each now an early return owning its own issue or warning), over a 13-line loop. **A second module the plan did not name — `donor_chemistry.py`.** The split forces it: `AA` and `DONOR_ELEMENTS` are read by the proximity path that stays in `bond_analysis` *and* by the declaration path that leaves, so leaving them behind would have made `declared_connections` import the module that imports it. It takes the whole donor vocabulary, including `INFERRED_DONOR_ATOMS` and the cross-check that keeps the two tables in step. The decomposition is verified by differential execution rather than by eye: HEAD's implementation was run beside the new one across the whole offline suite, 174 calls, every return identical. **The two-phase metal check was not in fact protected** — 2.3 says it is (`test_declared_connections.py:1221`), but deleting either phase, or dropping the flag when resolution raises, left the offline suite green. All three now fail: two unit tests over `_resolve_declared_partners` pin the phase that runs *before* `find_cra`, and a cluster test pins the one after it (`FES`/`FE1` names no metal on its face, so only the resolved atom's element makes an unresolvable iron-sulfur coordination reportable). A fourth survivor — the guard dropping contacts inside the metal's own residue — is caught now that the intra-cofactor test asserts no *candidate* either, not just no bond row. `bond_analysis.py` 1,974 → 1,564 lines. Seven mutations tried, all seven caught.
| 20 · `Extract the bond output schema and row builders` | `fcd9ca8` | `bond_schema.py` owns the three published CSV schemas, the four builders that fill them, and `_check_row_schema`, which moves out of `worker.py` — the module that defines a column list is now the one that enforces it, and `driver.writers` no longer reaches into the worker to borrow the check. All twelve definitions are AST-identical. The builders are pure serialization: every decision they report was made in `bond_analysis` before the row exists, which is what lets the schema sit underneath the analysis instead of beside it. `ZSCORE_OUTLIER_CUTOFF` moves with them and `bond_analysis` imports it back — both CSVs publish it as a column, so a consumer reading an old file can see which threshold produced its `geometry_outlier` values. `bond_analysis.py` 1,564 → 1,005 lines, `worker.py` 615 → 588; no module in `src/` now exceeds ~1,200. Ten mutations tried, nine caught, and closing three of them cost three test edits: the schema-drift tests asserted only that *some* `RuntimeError` named the file, so a guard that stopped naming the drifted column passed — they now check the `missing X` / `unexpected X` clause, which is the guard's whole value. And the 6-decimal rounding on published image positions was assumed by an existing tolerance comment but asserted nowhere, in either stream. **The survivor is not a gap.** Deleting the `summary`-wins update in `stats_extra_values` changes nothing measurable: the three columns it can override — `dpi_atom_count_multiplier`, `strict_ncs_operation_count`, `crystallographic_operation_count` — are read from the same `StructureContext` attributes on both sides, so no real call can tell the two apart. The line is left alone rather than deleted; that is a behavioural question, and this commit is a move.
| 21 · `Introduce typed worker and density contracts` | `e1bf8f8` | `WorkerConfig` (frozen, 16 fields — 14 when the review counted, before the timeout and log-level options) replaces the dict the driver pickles into every worker, and `DensityResult` replaces `run_density_analysis`' bare return, which the worker read with a mix of `res["…"]` and `res.get(…)` in one block. Frozen is the point on the config: each worker holds its own unpickled copy, so an edit would make one process behave differently from the rest for one entry with nothing in the manifest to say so. **Both hand-built copies of the config had already drifted** — neither test module's carried `log_level`, and one was missing `ccp4_timeout_s`. That is invisible in a dict and impossible in a dataclass, which is most of the argument for this commit. Eight mutations tried, all eight caught, but only after closing three real gaps: `_worker_config` — the one hand-written mapping from CLI options to config fields, where `keep=args.bonds` would type-check perfectly — had **no test at all** outside the entry-data lane that does not run in CI; and nothing checked that the worker reports the scope the density stage *achieved* rather than the one requested, that its per-program timings are merged, or that `--keep-intermediates` reaches the worker's scratch cleanup. The timeout harness in `test_driver_manifest` generalizes into `_manual_entry`, which now drives a *successful* stubbed density stage as well as a failing one. **A note for whoever runs mutations next.** Restoring a mutated file within the same second as the mutant's compile leaves CPython reusing the mutant `.pyc` — source mtime is cached at one-second granularity — and every later mutation then looks caught by whatever the stale bytecode broke. Run the suite with `-B` and `PYTHONDONTWRITEBYTECODE`. Commits 19 and 20 were re-verified under those conditions and their recorded results stand.
| 22 · `Introduce a typed entry result` | `6f8a261` | `EntryResult` replaces the 36-key dict every stage mutates, every worker pickles back, and the manifest writer, CSV writers, confidence stage, run log and resume check all read. Mutable by necessity — it is filled in stage by stage — so the guarantees come from elsewhere: `update()` rejects a name the result does not have, where a dict's `update` used to accept `retryble=True` and leave the entry reporting whatever `retryable` still held. **The blank-vs-zero invariant is now the type.** `n_bonds`/`n_candidates` are `Optional[int]` starting at `None`, not `""` in a field otherwise holding integers, and `blank_if_unmeasured` renders `None` back to the empty cell every output has always written. The three model fields get the same treatment for the same reason. `pdbID`/`n`/`runtime` become `pdb_id`/`n_metals`/`runtime_s`, which finishes commit 9's rule for this dict — the CSV column keeps the deposited spelling, mapped once in `writers.MANIFEST_FIELDS`, and the projection now reads fields by name with no silent `""` default, so a column that loses its field raises instead of writing a blank. **Verified by differential output, not by eye**: six entry outcomes × three manifest projections, plus each one's run-log entry, dumped from HEAD and from this commit — byte-identical. The entry-data lane then exercised the real pickling boundary. Eight mutations tried, all eight caught, two of them only after new tests: nothing had pinned `update`'s name check (it is new), and **nothing asserted the model provenance ever stops being blank** — `input_model_count`, `model_analyzed` and `multi_model_structure` could have been left unmeasured for every entry in a database run with the suite green.
| 23 · `Introduce a typed candidate record and a shared code vocabulary` | `3387e18` | `contact_record.Candidate` replaces the dict that discovery creates and four later stages annotate in place — 40 fields, `slots=True` so `candidate.eligibilty = x` is an error at the assignment rather than an attribute nobody reads. The `.get(name, default)` calls the review flagged become field defaults in one place; `candidate.update(...)` becomes plain assignment, which a type checker can see. `codes.py` gives the status and provenance words one definition: `GeometryStatus`, `MultiDonorStatus`, `ContactScope`, `CandidateSource`, `OccupancyStatus`, `ElementStatus`, `WarningCode`. **The two `suspect`s stay equal as strings and become different names** — one is a site verdict, one a donor-group verdict — which is what 3.5 asked for. `insufficient data` keeps its space: it is a published column value, and changing it would silently reclassify every site in an existing output file. `OccupancyStatus` turned out to have a seventh value the review did not list, `invalid_value`, reachable only for an mmCIF-sourced atom with no raw PDB record. **No output changes**, verified twice over: six analysed structures × their bond rows, candidate rows, site summaries and metadata, plus the manifest and run-log dumps from commit 22 — all byte-identical to HEAD. Ten mutations tried, eight caught. **Both survivors are defaults the dict invented and nothing ever reached.** `inferred_donor_allowed` defaults permissive, but `run_bond_analysis` annotates every candidate before the first-sphere stage reads it; the copy `_merge_candidates` makes is defensive against a declared candidate being aliased into two scopes, and no constructible input observes it, because an explicit declared contact annotates identically in both. Both are documented as unreachable rather than tested or deleted — deleting either would change what an out-of-order caller does. A third survivor was fixed instead: `candidate_sources` had a `default_factory` no caller used, and is now required. **Follow-up after review:** `EntryResult.update(**fields)` is gone — it took every value as `Any`, so `n_bonds="0"` passed it and broke the blank-versus-measured rule `--resume` depends on; the stages assign fields directly and `EntryResult` gains `slots=True`, which makes a misspelling an error at the assignment. `Candidate` now annotates `contact_scope`, `candidate_sources` and `multi_donor_geometry_status` with their enums rather than `str`. `WorkerConfig`'s freeze is documented as shallow: `env` and `manual_inputs` are still mutable dicts, and closing that needs a picklable immutable mapping, which `MappingProxyType` is not.
| 24 · `Test and checksum the bundled reference data` | `b6119fe` | `tools/` had no tests at all; it has 27 now, over `element_counts`, `_biconnected_components` and `classify_component`, each against a CCD block built with real connectivity. Until now a change to the classification rules was checked only by rebuilding against the 40,000-component CCD, which needs a download and cannot run in CI — and `parent_type` is `cluster`, `heme` or neither for every metal site in the database. Both bundled files are now verified when first read, so a run stops rather than analyze against data that drifted from what its tool recorded. `metal_distances_info.txt` gets the sidecar it never had — checksum, row count, both citations, and the `CA` trap written down — plus `tools/stamp_distance_table.py` to rewrite it, because unlike the catalog nothing generates that table. Only the two bundled paths are checked: a path a caller passes is theirs. **The distance-table parser is strict now.** It skipped any row it could not read, which turned a typo in a reference distance into a contact 'no reference covers' — indistinguishable from a metal-donor pair genuinely absent from the literature. It also accepted a sixth column, silently replacing that pair's real numbers. Both are errors naming the line. The bundled file is untouched: 2.5's question about whether editing it should invalidate frozen references is still open, and re-stamping is now the moment that decision gets made. Eleven mutations tried, all eleven caught; two needed tests that were missing rather than wrong — the porphyrinoid core-size floor was only ever exercised through the carbon floor beside it, so it could be deleted with every heme test still green.
| 25 · `Record the reference-data identity in the manifest` | `58184bf` | `reference_data_id` — twelve hex characters over both bundled files' checksums — is resolved once per run, carried on `WorkerConfig` and `EntryResult` like the other provenance, and written to every manifest row. One id rather than two columns because the question a consumer asks is whether two rows are comparable, and both files decide that: the catalog says what counts as a metal cofactor, the distance table sets every assignment cutoff and every z-score. The two component hashes go to the run log in full, so a changed id can be attributed to a file. It is composed from the files as read, not from the sidecars' recorded values, so it describes what the run actually measured against. **It breaks resume compatibility, deliberately.** A manifest written before this column does not say which reference data produced its rows, so appending rows that do would make one file two datasets with nothing to tell them apart. The refusal now names the columns that differ — it previously blamed a Gemmi migration for every mismatch, which is one cause among several, and an operator who upgraded mid-database was left diffing headers by hand. Six mutations tried, all six caught.
| 26 · `Tidy driver reporting and documentation` | *pending* | **One failure mechanism.** `DriverError` (public now — the CLI catches it too) is the driver's only one, and the two places that still raised `SystemExit` of their own — an unusable `--output-dir`, an unresolvable CCP4 installation — raise it instead. Argument validation is argparse's, all of it: `--id` with `--id-file` exited 1 while `--retry-partials` without `--resume` exited 2, the same class of mistake in the same function. Both exit 2 now, which is what argparse means by "you called this wrong". `run_logging` no longer exits on an unusable `--log-file`; it raises `OSError` and `cli.main` reports it, because that failure happens before any handler exists and only the caller knows how it can still be reported. **`--log-dir`, defaulting to `<output-dir>/logs/`.** One log per invocation accumulates forever beside the four CSVs a user actually opens; after a hundred runs the output directory is unreadable. **The README is 610 lines no longer.** It is 120: overview, quick start, dependencies, and links. `docs/usage.md`, `docs/method.md`, `docs/operations.md` and `docs/maintenance.md` carry the rest. Two new documentation tests guard the new failure mode a split introduces — every link resolves, and no page is orphaned — and the environment check now covers all five prose files rather than the README alone. `tests/README.md`'s two stale claims are corrected: the ledger has no `xfail` pins, and the command that ran it collected nothing. The ledger's own last stale sentence went with them — the finding it called open was fixed in `36e5dbf`. Six mutations tried, all six caught.

Suite: **1,085 passed, 26 skipped** offline, coverage **85.6%** (853 and untracked at review time). Lint, format and coverage are gates in CI.
...
**Still open:** the licence; whether editing `metal_distances_info.txt` should invalidate frozen references (2.5); and **the type checker commit 3 promised but never added** — `pyproject.toml` declares no `mypy`, and CI runs Ruff and pytest only, so every annotation Phase G introduced is unchecked outside an editor. It wants its own commit: adding the gate means adopting whatever baseline it reports across twelve modules.

Phase E is done: `main.py` is 3,126 lines no longer, and no module in `src/` exceeds ~1,220. Phase F is done too: `bond_analysis.py` is 2,204 lines no longer, and the six subsystems 2.3 identified are six modules. Phase G is done: the three cross-process contracts are declared types, and the status words have one definition. Phase H is done, and with it the plan. What remains is what the plan deliberately left: the licence, whether editing `metal_distances_info.txt` should invalidate frozen references (2.5), and the type-check gate commit 3 promised.
...
**On revertibility.** Calling these "pure moves, individually revertible" would be too strong. Genuinely pure moves are commits 11–14, 16, 17 and 20. The rest are not: the `_run` decomposition in 15, lazy reference loading and parser unification in 18, the `_collect_declared_candidates` decomposition in 19 — which 2.3 asks for, and which is output-identical but is not a move — and every dataclass and enum commit in Phase H. And once downstream commits land, none reverts cleanly in isolation — they are dependency-ordered. Treat them as **staged behavioural refactors with focused regression gates**: each needs its own test before the next lands.
