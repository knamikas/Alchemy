...
Phase E is done: `main.py` is 3,126 lines no longer, and no module in `src/` exceeds ~1,220. Phase F is done too: `bond_analysis.py` is 2,204 lines no longer, and the six subsystems 2.3 identified are six modules. Phase G is under way; next is commit 23, the candidate record and the shared code vocabulary.
