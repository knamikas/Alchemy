...
def _stats_row(pdb_id: str = "1abc", zdm: object = 3.0, zd_neg: object = -1.0,
               zd_pos: object = 2.0, **overrides):
...
def _bond_row(pdb_id: str = "1abc", covered: bool = True,
              zscore: float | None = 1.5, neighbor: str = "HIS",
              atom: str = "NE2", **overrides):
...
DENSITY_EXPECTED = {
    ...
    2.5: 0.125, 3.0: 0.25,
    4.5: 0.25 + 0.5 * 0.40, 6.0: 0.65,
    9.0: 0.65 + 0.5 * 0.35, 12.0: 1.0, 40.0: 1.0,
}
GEOMETRY_EXPECTED = {
    0.0: 0.0, 2.0: 0.0,
    2.5: 0.175, 3.0: 0.35,
    4.5: 0.35 + 0.5 * 0.35, 6.0: 0.70,
    9.0: 0.70 + 0.5 * 0.30, 12.0: 1.0, 40.0: 1.0,
}


def _expected_confidence(sr, sg, qg):
    """README formula, written out independently of the implementation."""
    return 100.0 * (1.0 - 0.50 * sr - 0.35 * qg * sg
                    - 0.15 * qg * math.sqrt(sr * sg))
...
@pytest.mark.parametrize("value", [-1e-9, -0.5, -50.0, math.nan, math.inf,
                                   -math.inf])
...
# score_site()
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("rszd,zbond,coverage,sr,sg", [
...
def test_score_site_matches_the_readme_formula(rszd, zbond, coverage, sr, sg):
    ...
    assert result["geometry_score_component"] == pytest.approx(
        0.35 * coverage * sg)
...
@pytest.mark.parametrize("rszd,zbond,coverage", [
    (math.nan, 6.0, 1.0),
    (-1.0, 6.0, 1.0),
    (math.inf, 6.0, 1.0),
    (3.0, math.nan, 1.0),
    ...
    for rszd in (0.0, 2.5, 6.0, 1e6):
        for zbond in (0.0, 2.5, 6.0, 1e6):
            for coverage in (-5.0, 0.0, 0.5, 1.0, 7.0):
                ...
                argument = (result["density_severity"]
                            * result["geometry_severity"])
...
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("covered_flags,expected_covered,expected_coverage", [
...
def test_geometry_coverage_is_reference_covered_over_assigned(
    ...
    bonds = [_bond_row(covered=flag, atom=f"N{index}")
...
def test_geometry_coverage_with_no_assigned_contacts_is_zero_not_an_error():
    ...
    assert "no_assigned_contacts" in prepared[
        "confidence_inputs_missing_reasons"].split("|")
...
def test_rejected_broad_search_candidates_stay_out_of_the_qg_denominator(
    ...
    bonds, candidates, _, _ = run_bond_analysis(
        "1abc", path, [], list(helpers.EDSTATS_HEADER), helpers.dpi_inputs(),
        structure=context)
    ...
    stats = _stats_row(**{key: str(bonds[0][key]) for key in cs.SITE_KEY_COLUMNS
                          if key != "pdbID"})
...
def test_finite_zbond_count_is_recorded_apart_from_reference_coverage(tmp_path):
    ...
    bonds = [_bond_row(covered=True, zscore=None, atom="NE2"),
    ...
    reference = cs.write_reference(
        str(tmp_path / "ref"), Counter({50.0: 3}), 3)
...
def test_max_abs_zbond_takes_the_largest_magnitude_and_names_its_contact():
    ...
    bonds = [
        ...
        _bond_row(zscore=-7.5, neighbor="ASP", atom="OD1",
                  neighbor_chain="C", neighbor_resnum="42"),
...
def test_prepare_emits_one_row_per_selected_site_and_skips_unselected():
    ...
    other = _stats_row(pdb_id="1abc", metal_atom_index="7",
...
@pytest.mark.parametrize("zdm,expected_status,expected_reason", [
    ("", "unscorable", "rszd_unavailable"),
    ("n/a", "unscorable", "rszd_unavailable"),
    ("-4.0", "complete", ""),
])
def test_missing_rszd_makes_a_site_unscorable(zdm, expected_status,
                                              expected_reason):
    ...
    reasons = [r for r in prepared[
        "confidence_inputs_missing_reasons"].split("|") if r]
...
def test_partial_geometry_coverage_is_flagged_not_silently_averaged():
    ...
    bonds = [_bond_row(covered=True, atom="NE2"),
             _bond_row(covered=False, atom="CB")]
    prepared, = cs.prepare_confidence_inputs([_stats_row()], bonds)
    assert prepared["confidence_inputs_status"] == "partial_geometry"
...
def test_declared_inferred_and_multi_donor_contacts_are_counted():
    ...
    bonds = [
        _bond_row(atom="NE2", declared_connection="True",
                  coordination_status="declared"),
        _bond_row(atom="OD1", coordination_status="inferred",
                  multi_donor_detected="True"),
        _bond_row(atom="OD2", coordination_status="inferred",
                  multi_donor_detected="True"),
    ]
    prepared, = cs.prepare_confidence_inputs([_stats_row()], bonds)
...
def test_result_preparation_agrees_with_row_preparation():
    ...
    flat = _stats_row()
    bonds = [_bond_row(atom="NE2", zscore=4.0),
...
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("prepared_count,selected", [(0, 3), (1, 2), (2, 2),
                                                     (0, 0)])
def test_every_manifest_counted_site_keeps_exactly_one_row(prepared_count,
                                                           selected):
    """The streamed cohort holds exactly one row per manifest-counted metal."""
    rows = [_input_row(metal_atom_index=str(index))
    ...
    placeholders = [row for row in completed
                    if row["selected_metal_site_status"]
                    == "selected_site_unresolved"]
    ...
    assert len({row["metal_atom_index"] for row in placeholders}) == len(
        placeholders)
...
def test_missing_reason_downgrades_real_rows_and_is_not_duplicated():
    ...
    assert completed[0]["confidence_inputs_missing_reasons"].split("|") == [
        "already", "bond_failure"]
    assert "bond_failure" in completed[1][
        "confidence_inputs_missing_reasons"].split("|")
...
def test_reference_id_is_deterministic_and_tracks_the_distribution(tmp_path):
    ...
    assert len({first.reference_id, different_count.reference_id,
                different_score.reference_id, extra_score.reference_id}) == 4
...
def test_small_runs_take_percentiles_from_the_frozen_cohort_only(tmp_path):
    ...
    row = _input_row(rszd_magnitude="3", max_abs_zbond="6",
                     geometry_coverage="1")
    ...
    batch = [_input_row(rszd_magnitude=str(value), max_abs_zbond="12",
                        geometry_coverage="1")
             for value in (2.0, 2.0, 12.0, 12.0, 12.0)] + [row]
    scored = cs.score_against_reference(batch, reference)
    assert scored[-1]["confidence_percentile"] == alone[0][
        "confidence_percentile"]
    assert {int(r["confidence_cohort_size"]) for r in scored} == {100}
    assert {r["confidence_reference_id"] for r in scored} == {
...
def test_scoring_status_distinguishes_density_only_from_partial_geometry(
    ...
    assert [row["confidence_scoring_status"] for row in scored] == [
        "complete", "partial_geometry", "density_only", "unscorable"]
    assert scored[1]["confidence_scoring_reason"] == (
        "geometry_coverage_below_one")
...
def test_context_warning_is_carried_through_without_changing_the_score(
    ...
    reference = _frozen_reference(tmp_path)
    plain = _input_row()
    warned = _input_row(context_warning="True",
                        context_warning_reasons="suspect_multi_donor_group")
    scored_plain, scored_warned = cs.score_against_reference(
        [plain, warned], reference)
...
def test_finalize_builds_the_cohort_from_scorable_rows_only(tmp_path):
    ...
    counted."""
    rows = [
        _input_row(metal_atom_index="0", rszd_magnitude="3",
                   max_abs_zbond="6", geometry_coverage="1"),
        _input_row(metal_atom_index="1", rszd_magnitude="3",
                   max_abs_zbond="6", geometry_coverage="1"),
        _input_row(metal_atom_index="2", rszd_magnitude="2",
                   max_abs_zbond="2", geometry_coverage="1"),
        _input_row(metal_atom_index="3", rszd_magnitude=""),
        _input_row(metal_atom_index="4", rszd_magnitude="3",
                   max_abs_zbond="", geometry_coverage="1"),
    ]
...
def test_finalize_removes_a_stale_reference_when_it_cannot_publish(tmp_path):
    ...
    input_path = _write_input_csv(
        tmp_path / "in.csv", [_input_row(rszd_magnitude="")])
    with pytest.raises(ValueError, match="no scores"):
...
def test_finalize_is_reproducible_for_the_same_inputs(tmp_path):
    ...
    assert first == second
    assert (cs.load_reference(str(tmp_path / "ref_a")).reference_id
            == cs.load_reference(str(tmp_path / "ref_b")).reference_id)
    assert (tmp_path / "a.csv").read_text(encoding="utf-8") == (
        tmp_path / "b.csv").read_text(encoding="utf-8")
...
def test_validate_scored_reference_accepts_output_from_the_same_reference(
    ...
    assert [row["confidence_reference_id"] for row in rows] == [
        reference.reference_id]
    ...
    other = cs.write_reference(
        ...
        Counter({float(value): 1 for value in range(1, 51)}), 50)
...
def test_validate_scored_reference_rejects_another_database_snapshot(tmp_path):
    ...
    other = cs.write_reference(
        str(tmp_path / "ref_b"), Counter({float(v): 1 for v in range(1, 51)}),
        50)
