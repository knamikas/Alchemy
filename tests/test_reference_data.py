...
def _catalog(tmp_path: Path, lines: Iterable[str], name: str = "catalog.txt") -> str:
    path = tmp_path / name
...
def test_every_accessor_rejects_the_same_bad_catalog(
...
def test_first_sphere_targets_is_the_longest_distance_per_metal_and_donor() -> None:
...
def test_an_edited_bundled_file_is_refused_at_load(
...
def test_a_caller_supplied_file_is_not_checksummed(tmp_path: Path) -> None:
...
def test_a_damaged_distance_row_fails_the_parse(
...
def test_distance_values_must_be_finite_and_positive(
    tmp_path: Path, mu: str, stdev: str, message: str
) -> None:
    table = tmp_path / "distances.txt"
...
def test_the_identity_is_stable_for_unchanged_data(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_the_identity_verifies_before_it_reports(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_the_unambiguous_ids_still_cover_the_metals_that_matter() -> None:
