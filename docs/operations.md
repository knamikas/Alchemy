...
| `bond_stage_failure` | The geometry stage raised after density analysis, with an exception that does not describe the entry's data. The density rows are kept and the entry is `partial` with no assigned contacts, so its sites are density-only evidence. A parse, lookup, or arithmetic error in the geometry stage instead ends the entry as `deterministic_processing_error`, like the same error in any other stage. |
