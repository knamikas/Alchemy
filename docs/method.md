...
donors have literature references and which do not, and what the score does and
does not claim.
...
geometric proof of coordination. A declared partner whose element is not `N`,
...
score calculation. Machine-readable `context_warning_reasons` explain the
...
The Alchemy score (0–100) ranks available density and geometry evidence against
frozen reference distributions. Higher scores indicate more typical behavior
relative to the reference cohort, while lower scores indicate increasingly
unusual disagreement. It is not a probability of correct metal assignment.

### Database-referenced scoring

Score inputs are collected as part of an uncapped full-database run: no
...
to `score_inputs_all.csv`. It never rereads the complete statistics and
...
conditions don't enter a raw classification threshold, empirical reference
distribution, the overall verdict matrix, or `alchemy_score`.

After score finalization, Alchemy joins the entry summary to only `REVIEW`
...
primary site and score outputs.
