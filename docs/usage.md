...
| `--workers <n>` | Override multiprocessing parallelism; must be at least 1. By default, Alchemy uses the lower CPU or available-memory limit, budgeting 1.25 GiB per worker, and never more workers than entries. Automatic sizing reads the machine's free memory and does **not** detect a cgroup memory limit, which is what a container or an HPC scheduler such as SLURM imposes: pass `--workers` explicitly under one, or the run may size itself from memory it is not permitted to use. |
